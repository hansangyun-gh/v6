...
export interface EvaluationResult {
  ...
  score: number;
  summary: string;
  parsedSummary?: any[]; // robust하게 추출된 JSON 배열
}
...
const ResultsDisplay: React.FC<ResultsDisplayProps> = React.memo(({ results }) => {
  ...
  // parsedSummary 우선 사용
  let parsed: any[] = results.flatMap(r => {
    if (Array.isArray(r.parsedSummary) && r.parsedSummary.length > 0) {
      return r.parsedSummary.map((item: any) => ({ ...item, _filename: r.id }));
    }
    // fallback: summary에서 기존처럼 파싱 시도
    try {
      const arr = JSON.parse(r.summary);
      if (Array.isArray(arr)) return arr.map((item: any) => ({ ...item, _filename: r.id }));
      return [];
    } catch {
      return [{ _filename: r.id, 총점: r.score, 피드백: r.summary }];
    }
  });
  ...
  return (
    ...
    >
      ...
      {parsed.length === 0 ? (
        ...
        <>
          <Table variant="simple" size="md" bg={tableBg} borderRadius="lg" boxShadow="sm">
            <Thead>
              <Tr>
                <Th>파일명</Th>
            ...
            <Tbody>
              {parsed.map((r, idx) => (
                <Tr key={`${r._filename}-${idx}`}>
                  <Td fontWeight="bold">{r['제목'] || r._filename}</Td>
                  {SCORE_FIELDS.map(field => (
                    <Td key={field} color={getScoreColor(Number(r[field]))} fontWeight={field === '총점' ? 'bold' : 'normal'}>
                      {typeof r[field] === 'number' || /^[0-9]+$/.test(r[field]) ? r[field] : '-'}
                    </Td>
                  ))}
                  <Td>
                    {r['피드백'] ? (
                      <Tooltip label={r['피드백']} aria-label="피드백 상세">
                        <IconButton
                          ...
                          size="sm"
                          aria-label="피드백 보기"
                          onClick={() => { setModal({ title: r['제목'] || r._filename, feedback: r['피드백'] }); onOpen(); }}
                        />
                      </Tooltip>
                    ) : '-'}
                  </Td>
                  <Td>
                    <Tooltip label="LLM 원본 전체 보기" aria-label="LLM 원본 전체 보기">
                      <IconButton
                        ...
                        onClick={() => {
                          // summary는 results에서 id로 찾아야 함
                          const found = results.find(res => (res.id === r['제목'] || res.id === r._filename));
                          setLlmModal({ title: r['제목'] || r._filename, summary: found?.summary || '' });
                          onOpen();
                        }}
                      />
                    </Tooltip>
            ...
            <HStack spacing={2} align="flex-end">
              {parsed.map((r, idx) => (
                <Box key={`bar-${r._filename}-${idx}`} textAlign="center">
                  ...
                  <Text fontSize="xs" mt={1}>{r['제목'] || r._filename}</Text>
                  <Text fontSize="xs" color={getScoreColor(Number(r['총점']))}>{r['총점']}</Text>
                </Box>
              ))}
            </HStack>
          </Box>
